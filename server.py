...
def request_model(messages: list[dict[str, str]], *, temperature: float, max_tokens: int, json_mode: bool = True) -> dict[str, Any]:
    body = {
        "model": AI_MODEL,
        "messages": messages,
    ...
    try:
        with urllib.request.urlopen(request, timeout=AI_TIMEOUT) as response:
            ...
            log_event(
                ...
                f"preview={json.dumps(str(content)[:300], ensure_ascii=True)}"
            )
            return payload
        ...
        try:
            ...
            pass
        log_event(f"DeepSeek HTTP {exc.code} elapsed={time.monotonic() - started:.2f}s detail={detail[:160]}")
        raise RuntimeError(f"DeepSeek接口返回 {exc.code}，请稍后重试。") from exc
    except urllib.error.URLError as exc:
        log_event(f"DeepSeek connection error elapsed={time.monotonic() - started:.2f}s reason={exc.reason}")
        raise RuntimeError("DeepSeek连接超时或网络不可用，请稍后重试。") from exc
    except TimeoutError as exc:
        log_event(f"DeepSeek timeout elapsed={time.monotonic() - started:.2f}s")
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802
        if self.path != "/api/chat":
            ...
            return
        try:
            log_event(f"POST received path={self.path} content_length={self.headers.get('Content-Length', '')}")
            length = int(self.headers.get("Content-Length", "0"))
            if length <= 0 or length > 1_000_000:
                raise ValueError("请求内容为空或过大")
            self.connection.settimeout(15)
            raw = self.rfile.read(length)
            log_event(f"POST body read bytes={len(raw)} expected={length}")
            if len(raw) != length:
            ...
            log_event("POST body timeout")
            self.send_json(408, {"error": "本地请求读取超时，请重新发送。"})
        except (ValueError, RuntimeError) as exc:
            log_event(f"chat error: {exc}")
            self.send_json(400, {"error": str(exc)})
        except Exception as exc:  # keep local demo responsive while surfacing a safe error
